...
function ShoesAdmin() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const shoesCollectionRef = collection(db, "נעליים");
        const snapshot = await getDocs(shoesCollectionRef);
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "נעליים", productId));
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "נעליים", productId));
      if (productDoc.exists()) {
        ...
        setEditFormData({
          ...
          isShoe: productData.isShoe || true,
        });
      }
    } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "נעליים", editingProduct);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على أحذية. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
