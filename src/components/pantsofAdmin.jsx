...
function PantsAdmin() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const pantsCollectionRef = collection(db, "מכנסיים");
        const snapshot = await getDocs(pantsCollectionRef);
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "מכנסיים", productId));
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "מכנסיים", productId));
      if (productDoc.exists()) {
        ...
        setEditFormData({
          ...
          pantsSizes: productData.pantsSizes || [],
          isPants: true, // Always set isPants to true for new/edited items
        });
      }
    } catch (error) {
  ...
  const handleSizeChange = (size) => {
    const { pantsSizes } = editFormData;
    if (pantsSizes.includes(size)) {
      setEditFormData({
        ...editFormData,
        pantsSizes: pantsSizes.filter((s) => s !== size),
      });
    } else {
      setEditFormData({
        ...editFormData,
        pantsSizes: [...pantsSizes, size],
      });
    }
  };
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "מכנסיים", editingProduct);
      ...
      await updateDoc(productRef, {
        ...
        pantsSizes: editFormData.pantsSizes,
        isPants: true, // Always ensure this field exists
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على بنطلونات. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                    ...
                    <tr key={product.id}>
                      {editingProduct === product.id ? (
                              ...
                              <div className="flex flex-wrap gap-3">
                                {availableSizes.map((size) => (
                                  ...
                                  >
                                    <input
                                      type="checkbox"
                                      checked={editFormData.pantsSizes.includes(
                                        size
                                      )}
                        ...
                        <>
                              ...
                              {product.imageUrl ? (
                                <img
                                  ...
                                  onError={(e) => {
                                    ...
                                    e.target.src = "/placeholder-pants.jpg";
                                  }}
                                />
                              ) : (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">
                              {formatSizes(product.pantsSizes)}
                            </div>
                          </td>
