...
function PantsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const pantsData = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
            name: data.name || "",
            image: data.imageUrl || "/placeholder-pants.jpg",
            price: data.price || 0,
            regularPrice: data.regularPrice || null,
            onSale: data.regularPrice ? true : false,
            soldOut: data.isOutOfStock || false,
            sizes: data.pantsSizes || [],
            isPants: data.isPants || true,
            description: data.description || "",
          };
        });

        setProducts(pantsData);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-white">
      {/* Top announcement banner - Updated text */}
      <div
      ...
      {/* Secondary announcement banner - Updated text */}
      <div
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10">
              {products.map((product) => (
                <div key={product.id} className="group relative">
                  {/* Product content - Removed <a> tag wrapper */}
                    ...
                    <div className="flex items-baseline space-x-2">
                      {product.regularPrice && (
                        <p className="text-sm text-gray-400 line-through">
                          {product.regularPrice.toFixed(2)} NIS
                        </p>
                      )}
                      <p className="text-sm font-bold">
                        {product.price.toFixed(2)} NIS
                      </p>
                      {product.onSale && (
