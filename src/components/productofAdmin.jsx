...
function ProductOfAdmin() {
  ...
  // Define available sizes including XS and XXXL
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "חולצות", productId));
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "חולצות", productId));
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "חולצות", editingProduct);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على قمصان. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
