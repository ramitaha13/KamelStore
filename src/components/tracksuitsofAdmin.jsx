...
function TrainingAdmin() {
  ...
  const [editFormData, setEditFormData] = useState({
    ...
    description: "",
    imageUrl: "",
    regularSizes: [], // Using regularSizes for clothing sizes
  });
  const navigate = useNavigate();

  // Define available training clothing sizes
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const trainingCollectionRef = collection(db, "טרנינג"); // Changed collection name to טרנינג
        const snapshot = await getDocs(trainingCollectionRef);
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "טרנינג", productId)); // Changed collection name
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "טרנינג", productId)); // Changed collection name
      if (productDoc.exists()) {
        ...
        setEditFormData({
          ...
          regularSizes: productData.regularSizes || [], // Using regularSizes for training clothes
          isTraining: true, // Changed from isHat to isTraining
        });
      }
    } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "טרנינג", editingProduct); // Changed collection name
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على ملابس تدريب. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                              ...
                              {product.imageUrl ? (
                                <img
                                  ...
                                  onError={(e) => {
                                    ...
                                    e.target.src = "/placeholder-training.jpg";
                                  }}
                                />
                              ) : (
