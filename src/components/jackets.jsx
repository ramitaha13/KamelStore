  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const jacketsData = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
            name: data.name || "",
            image: data.imageUrl || "/placeholder-jacket.jpg",
            price: data.price || 0,
      ...
      <main className="flex-grow py-8 px-4 md:px-8 lg:px-16">
        <div className="max-w-7xl mx-auto">
          ...
          {!loading && products.length === 0 && (
            <div className="text-center py-10">
              <p>No jackets available at the moment.</p>
            </div>
          )}
        </div>
      </main>
