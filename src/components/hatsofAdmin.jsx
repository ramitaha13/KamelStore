...
function HatsAdmin() {
  ...
  const [editFormData, setEditFormData] = useState({
    ...
    description: "",
    imageUrl: "",
    regularSizes: [], // Changed from shoeSizes to regularSizes based on the Firestore schema
  });
  const navigate = useNavigate();

  // Define available hat sizes
  const availableSizes = ["S", "M", "L", "XL", "XXL"]; // Changed sizes to be appropriate for hats
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const hatsCollectionRef = collection(db, "כובעים"); // Changed collection name
        const snapshot = await getDocs(hatsCollectionRef);
        ...
        console.error("Error fetching hats:", error); // Updated error message
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "כובעים", productId)); // Changed collection name
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "כובעים", productId)); // Changed collection name
      if (productDoc.exists()) {
        ...
        setEditFormData({
          ...
          regularSizes: productData.regularSizes || [], // Changed from shoeSizes to regularSizes
          isHat: true, // Changed from isShoe to isHat
        });
      }
    } catch (error) {
  ...
  const handleSizeChange = (size) => {
    const { regularSizes } = editFormData; // Changed from shoeSizes to regularSizes
    if (regularSizes.includes(size)) {
      setEditFormData({
        ...editFormData,
        regularSizes: regularSizes.filter((s) => s !== size), // Changed from shoeSizes to regularSizes
      });
    } else {
      setEditFormData({
        ...editFormData,
        regularSizes: [...regularSizes, size], // Changed from shoeSizes to regularSizes
      });
    }
  };
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "כובעים", editingProduct); // Changed collection name
      ...
      await updateDoc(productRef, {
        ...
        regularSizes: editFormData.regularSizes, // Changed from shoeSizes to regularSizes
        isHat: true, // Changed from isShoe to isHat
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على قبعات. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                        ...
                        <>
                              ...
                              {product.imageUrl ? (
                                <img
                                  ...
                                  onError={(e) => {
                                    ...
                                    e.target.src = "/placeholder-hat.jpg"; // Changed placeholder image
                                  }}
                                />
                              ) : (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">
                              ...
                              {/* Changed from shoeSizes to regularSizes */}
                            </div>
                          </td>
