...
function NewCollectionAdmin() {
  ...
  const [editFormData, setEditFormData] = useState({
    ...
    description: "",
    imageUrl: "",
    shoeSizes: [], // Using shoeSizes based on the Firestore schema shown in the image
    regularSizes: [], // Added clothing sizes
    isShoe: true, // Default to shoe product type
  });
  const navigate = useNavigate();

  // Define available shoe sizes
  ...
  // Define available clothing sizes
  const availableClothingSizes = ["XS", "S", "M", "L", "XL", "XXL", "XXXL"];
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const productsCollectionRef = collection(db, "New Collection"); // Changed to "New Collection"
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المنتج؟")) {
      try {
        ...
        await deleteDoc(doc(db, "New Collection", productId));
  ...
  const handleEditClick = async (productId) => {
    try {
      const productDoc = await getDoc(doc(db, "New Collection", productId));
      if (productDoc.exists()) {
        ...
        setEditFormData({
          ...
          shoeSizes: productData.shoeSizes || [],
          regularSizes: productData.regularSizes || [], // Added clothing sizes
          isShoe: productData.isShoe !== undefined ? productData.isShoe : true,
        });
      }
    } catch (error) {
  ...
  // Toggle product type between shoe and clothing
  const handleProductTypeToggle = () => {
    setEditFormData({
      ...editFormData,
      isShoe: !editFormData.isShoe,
    });
  };

  // Handle shoe size checkboxes in edit form
  const handleShoeSizeChange = (size) => {
    const { shoeSizes } = editFormData;
    if (shoeSizes.includes(size)) {
      setEditFormData({
        ...editFormData,
        shoeSizes: shoeSizes.filter((s) => s !== size),
      });
    } else {
      setEditFormData({
        ...editFormData,
        shoeSizes: [...shoeSizes, size],
      });
    }
  };

  // Handle clothing size checkboxes in edit form
  const handleClothingSizeChange = (size) => {
  ...
  const handleSaveEdit = async () => {
    try {
      const productRef = doc(db, "New Collection", editingProduct);
      ...
      await updateDoc(productRef, {
        ...
        shoeSizes: editFormData.shoeSizes,
  ...
  const formatSizes = (product) => {
    if (product.isShoe) {
      return !product.shoeSizes || !Array.isArray(product.shoeSizes)
        ? "غير متوفر"
        : product.shoeSizes.join(", ");
    } else {
      return !product.regularSizes || !Array.isArray(product.regularSizes)
        ? "غير متوفر"
        : product.regularSizes.join(", ");
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {products.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-6 text-center">
            <p>لم يتم العثور على منتجات. يرجى إضافة منتجات جديدة.</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                    ...
                    <tr key={product.id}>
                      {editingProduct === product.id ? (
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-4 space-x-reverse">
                                <label className="inline-flex items-center">
                                  <input
                                    type="radio"
                                    checked={editFormData.isShoe}
                                    onChange={handleProductTypeToggle}
                                    className="form-radio h-5 w-5"
                                  />
                                  <span className="mr-2">حذاء</span>
                                </label>
                                <label className="inline-flex items-center">
                                  <input
                                    type="radio"
                                    ...
                                    className="form-radio h-5 w-5"
                                  />
                                  <span className="mr-2">ملابس</span>
                                </label>
                              </div>
                            </div>

                            {/* Sizes Section - Show based on product type */}
                            {editFormData.isShoe ? (
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                  مقاسات الأحذية
                                ...
                                <div className="flex flex-wrap gap-3">
                                  {availableShoeSizes.map((size) => (
                                    ...
                                    >
                                      <input
                                        type="checkbox"
                                        checked={editFormData.shoeSizes.includes(
                                          size
                                        )}
                                        onChange={() =>
                                          handleShoeSizeChange(size)
                                        }
                        ...
                        <>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">
                              {product.isShoe !== false ? "حذاء" : "ملابس"}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">
