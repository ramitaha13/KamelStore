...
function TracksuitsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const tracksuitsCollectionRef = collection(db, "טרנינג");
        const snapshot = await getDocs(tracksuitsCollectionRef);

        const tracksuitsData = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
            name: data.name || "",
            image: data.imageUrl || "/placeholder-tracksuit.jpg",
            price: data.price || 0,
      ...
      <main className="flex-grow py-8 px-4 md:px-8 lg:px-16">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10">
              {products.map((product) => (
                <div key={product.id} className="group relative">
                  {/* Removed <a> tag to prevent URL change on click */}
                  <div className="relative">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src = "/placeholder-tracksuit.jpg";
                      }}
                    />
                    {product.onSale && (
          ...
          {!loading && products.length === 0 && (
            <div className="text-center py-10">
              <p>No tracksuits available at the moment.</p>
            </div>
          )}
        </div>
      </main>
