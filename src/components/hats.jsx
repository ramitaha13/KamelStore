...
function HatsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const hatsCollectionRef = collection(db, "כובעים");
        const snapshot = await getDocs(hatsCollectionRef);

        const hatsData = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
            name: data.name || "",
            image: data.imageUrl || "/placeholder-hat.jpg",
            price: data.price || 0,
    ...
    <div className="min-h-screen flex flex-col bg-black text-white">
      ...
      <main className="flex-grow py-8 px-4 md:px-8 lg:px-16">
        <div className="max-w-7xl mx-auto">
          {/* Page title */}
          <h1 className="text-4xl font-normal mb-8 md:mb-12 px-6 py-4">Hats</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10">
              {products.map((product) => (
                <div key={product.id} className="group relative">
                  {/* Removed <a> tag wrapper to prevent URL changes */}
                  <div className="relative">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src = "/placeholder-hat.jpg";
                      }}
                    />
                    {product.onSale && (
